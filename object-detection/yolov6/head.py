...
def build_effidehead_layer_fuse_ab(
  channels_list, num_anchors, num_classes, reg_max=16, num_layers=3
):
...
class DetectAnchorBase(nn.Module):
  '''
  ...
  def forward(self, x):
    if self.training:
      ...
      for i in range(self.nl):
        ...
        if self.use_dfl:
          reg_output_af = reg_output_af.reshape([-1, 4, self.reg_max + 1, area]).permute(0, 2, 1, 3)
          reg_output_af = self.proj_conv(F.softmax(reg_output_af, dim=1))

        cls_output_af = torch.sigmoid(cls_output_af)
        cls_score_list_af.append(cls_output_af.reshape([b, self.nc, area]))
        reg_dist_list_af.append(reg_output_af.reshape([b, 4, area]))
