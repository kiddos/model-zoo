...
class COCODataset(Dataset):
  ...
  def load_entry(self, index, force_load_size=None):
    """Load image.
    ...
    # process labels to array of [[id, cx, cy, w, h], ...]
    label_arr = np.zeros([len(labels), 5])
    for i, label in enumerate(labels):
      x, y, w, h = label['bbox']
      category_id = self.category_mapping[str(label['category_id'])]
      cx = x + w / 2
      cy = y + h / 2
      label_arr[i, 0] = category_id
      label_arr[i, 1] = cx / w0
      label_arr[i, 2] = cy / h0
      label_arr[i, 3] = w / w0
      label_arr[i, 4] = h / h0
    return img, (h0, w0), img.shape[:2], label_arr
  ...
  def __getitem__(self, index):
    ...
    if self.augment and random.random() < self.hyp["mosaic"]:
      ...
      if self.hyp and "test_load_size" in self.hyp:
        img, (h0, w0), (h, w), labels = self.load_entry(index, self.hyp["test_load_size"])
      else:
        img, (h0, w0), (h, w), labels = self.load_entry(index)
  ...
  def get_mosaic(self, index):
    ...
    indices = [index] + random.choices(range(0, len(self.dataset)), k=3)  # 3 additional image indices
    random.shuffle(indices)
    imgs, hs, ws, labels = [], [], [], []
    for index in indices:
      img, _, (h, w), labels_per_img = self.load_entry(index)
      imgs.append(img)
      hs.append(h)
      ws.append(w)
