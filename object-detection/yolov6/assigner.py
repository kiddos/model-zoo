...
class TaskAlignedAssigner(nn.Module):
  ...
  def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes):
    ...
    overlaps = compute_iou(gt_bboxes, pd_bboxes)
...
class ATSSAssigner(nn.Module):
  ...
  def forward(self, anc_bboxes, n_level_bboxes, gt_labels, gt_bboxes, mask_gt, pd_bboxes):
    ...
    is_pos = torch.where(
      iou_candidates > overlaps_thr_per_gt.repeat([1, 1, self.n_anchors]), is_in_candidate,
    ...
    if pd_bboxes is not None:
      ious = compute_iou(gt_bboxes, pd_bboxes) * mask_pos
