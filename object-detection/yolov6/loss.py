...
from dist2bbox import dist2bbox
  ...
  def __call__(self, box1, box2):
    ...
    if self.iou_type == 'giou':
      ...
      shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w),
                             4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
...
class VarifocalLoss(nn.Module):
  ...
  def forward(self, pred_score, gt_score, label, alpha=0.75, gamma=2.0):
    ...
    with torch.cuda.amp.autocast(enabled=False):
      loss = (
        F.binary_cross_entropy(pred_score.float(), gt_score.float(), reduction='none') * weight
      ).sum()
...
class ComputeLoss(nn.Module):
  ...
  def forward(self, outputs, targets, epoch_num, step_num):
    ...
    try:
      if epoch_num < self.warmup_epoch:
        target_labels, target_bboxes, target_scores, fg_mask = self.warmup_assigner(
          anchors, n_anchors_list, gt_labels, gt_bboxes, mask_gt,
          pred_bboxes.detach() * stride_tensor
        )
      else:
        target_labels, target_bboxes, target_scores, fg_mask = self.formal_assigner(
          pred_scores.detach(),
          pred_bboxes.detach() * stride_tensor, anchor_points, gt_labels, gt_bboxes, mask_gt
        )
      ...
      if epoch_num < self.warmup_epoch:
        ...
        target_labels, target_bboxes, target_scores, fg_mask = self.warmup_assigner(
          _anchors, _n_anchors_list, _gt_labels, _gt_bboxes, _mask_gt, _pred_bboxes * _stride_tensor
        )
        ...
        target_labels, target_bboxes, target_scores, fg_mask = self.formal_assigner(
          _pred_scores, _pred_bboxes * _stride_tensor, _anchor_points, _gt_labels, _gt_bboxes,
          _mask_gt
        )
    ...
    loss_iou, loss_dfl = self.bbox_loss(
      pred_distri, pred_bboxes, anchor_points_s, target_bboxes, target_scores, target_scores_sum,
      fg_mask
    )
  ...
  def preprocess(self, targets, batch_size, scale_tensor):
    ...
    max_len = max((len(item) for item in targets_list))
    targets = torch.from_numpy(
      np.array(list(map(lambda x: x + [[-1, 0, 0, 0, 0]] * (max_len - len(x)),
                        targets_list)))[:, 1:, :]
    ).to(targets.device)
  ...
  def bbox_decode(self, anchor_points, pred_dist):
    if self.use_dfl:
      ...
      pred_dist = F.softmax(pred_dist.view(batch_size, n_anchors, 4, self.reg_max + 1),
                            dim=-1).matmul(self.proj.to(pred_dist.device))
    return dist2bbox(pred_dist, anchor_points)


class ComputeLossAnchorBase(nn.Module):
  '''Loss computation func.'''
  ...
  def __init__(
    ...
    super(ComputeLossAnchorBase, self).__init__()
    ...
    self.warmup_epoch = warmup_epoch
    self.formal_assigner = TaskAlignedAssigner(
      topk=26, num_classes=self.num_classes, alpha=1.0, beta=6.0
    )
